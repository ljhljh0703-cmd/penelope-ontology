...
| submission-ready | Every required Build Week field and public artifact is verified against the final commit | blocked | `npm run submission:check` must pass; **Penelope Ontology**, the final description, and README are prepared locally, while exact-SHA release certification, public remote/CI, hosted demo, narrated video, private feedback ID, and confirmed Devpost track remain |
...
| public-readme | README reflects the current product, setup, Codex/GPT-5.6 use, and claim boundaries | local candidate | manually truth-audited from current repository evidence after the optional writing adapter reported `SERVING_STALE`; final status requires exact-SHA release verification |
