...
| health-readiness-binding | `/api/health` reports live evidence only when the generated readiness artifact has the full verified authority and privacy shape | verified | strict evidence type/model/hash/path/authority/privacy guard plus malformed-artifact tests |
...
| fresh-copy-rehearsal | A clean public candidate copy installs and passes the release gate | historical | prior implementation proof `fbb1b50497bff67eb6e83467cbdabc579a1c87c2` passed; final candidate record is generated only after its commit exists |
| deployment-smoke-local | The production smoke rejects stale build identity and verifies the exact two-step authority chain | historical | prior implementation proof `fbb1b50497bff67eb6e83467cbdabc579a1c87c2` passed; final candidate must receive a new cache-busted exact-SHA record |
