...
Status snapshot: **2026-07-22 KST**
Current state: **Penelope Ontology now has a portable World Pack runtime around its world-first Book 19 vertical slice. Book 19 remains the main causal proof; an Oz Chapter XV pack replaces its cast, facts, actions, reactions, and endings on the same workbench; World Forge compiles a creator-reviewed two-to-three-sentence seed and 24 approved facts into a five-scene episode with one bounded dynamic relationship. From one Book 19 checkpoint, the creator can dismiss Melantho through a goal/motive/cost interview and reach `Plan Compromised`, or preserve the containment route and reach `Canon Contained`. The Loom distinguishes resolving, review, accepted, and failed states; World Aftermath shows receipt-backed consequences; World Codex retains the approved episode spine, realized beats, dynamic relationships, ending conditions, and actual checkpoint tree without inferring from prose. Fate Frame adds a limited-color ASCII candidate at approved high-value triggers without making it canon automatically. A local Codex CLI proof requested `gpt-5.6-terra` and completed four accepted narration turns after two fail-closed wording rejections. The transport did not report the serving-model identity, and automated harness acceptance is not a human literary verdict. The public video, private `/feedback` field, and authenticated Devpost submission are complete. Exact commit, CI, deployment, and hosted-smoke evidence remains externalized in the ignored release record.**
...
→ deterministic NPC/world reaction
→ scoped narration candidate and approval
→ earned ending
...
| Devpost project page | verified and submitted | authenticated owner readback confirms the intended project name, Work & Productivity category, repository, hosted demo, public video, and private feedback field |
| Codex credits | user-reported received | $100 Codex credits are available for Codex; API-platform credit/key applicability remains unverified |
...
| World Workbench | verified on current tree | `/` and `/world` run the Book 19 causal demo with A/B/C, hash-confirmed C execution, truthful Loom states, World Aftermath, cumulative World Codex, causal NPC cards, preserved checkpoints, two endings, Fork Compare, five-scene World Forge with dynamic relationship state, and candidate-only Fate Frames |
| Portable World Packs | verified on current tree | two registered public-domain packs plus strict creator JSON import; canonical digest, checkpoint binding, ID-collision rejection, creator-only hidden-premise projection, temporary 30-minute server-memory retention, and desktop/mobile no-Odyssey-leakage browser coverage |
| World causal browser flow | verified on current tree | desktop Chromium and mobile WebKit cover C's no-checkpoint-before-confirmation rule, Melantho movement and pressure, `Plan Compromised`, return to opening, `Canon Contained`, World Codex readability, The Loom, World Aftermath, and state-based Fork Compare |
| World Codex CLI narration | generated; serving identity and human verdict open | requested `gpt-5.6-terra`; four final narration turns accepted unchanged after two fail-closed wording rejections. Typed state and checkpoints remained unchanged on rejection; no human literary-quality claim is made |
...
| Production browser flow | verified on current tree | the full 46-check Chromium desktop/mobile WebKit suite passes and includes the real Story fixture API path, Book 19 causal flow, truthful narrow-screen Loom failure, World Codex desktop/mobile checks, Oz switch, complete five-scene World Forge, dynamic relationship state, Fate Frame, and creator-owned starter import; route-mocked interaction checks remain separate |
...
| Clean-worktree release gate | verified per public release | evidence generation/verification, lint, typecheck, tests, privacy scan, production build, and browser checks run in GitHub CI; the exact SHA and run belong in the ignored release record |
| Production visual inspection | verified current tree | the Book 19 desktop and 390px mobile flows show scene prose, World Aftermath, The Loom, World Codex, NPC state, creator control, and Fork Compare without horizontal overflow or clipped relationship text |
| Fresh-copy rehearsal | verified on public release; per-commit result externalized | GitHub CI reinstalls from the lockfile and reruns evidence, lint, typecheck, Vitest, privacy, identified production build, and both browser projects; the resulting SHA belongs in the ignored release record because this tracked row cannot self-certify its own commit |
| Deployment smoke | verified on public host; per-commit result externalized | `https://penelope-ontology.vercel.app` requires no account and passes cache-busted exact-SHA identity, security-header, typed health-signal, fixture-authority, approved-overlay replay, two-step transition, and public-live denial checks |
| Independently verified GPT-5.6 | blocked | the Responses path has not run. The Story and World workbenches prove Codex CLI generation requesting `gpt-5.6-terra`, but that transport leaves `actualModel` and `responseId` null. The historical approval-bound CLI attempts remain terminal failures and cannot be rewritten as success |
...
| Release evidence | verified per release | seven sanitized evidence artifacts plus manifest exist; the current README was manually truth-audited from repository evidence; public remote, CI, hosted deployment, video, private `/feedback`, and final submission are bound through the ignored release record |
| Submission readiness gate | verified per release | `submission:check` recomputes local SHA and canonical child manifests, verifies the sanitized evidence bundle, enforces ignored private-record paths, reruns privacy, and live-checks public remote HEAD, the exact GitHub CI push run, hosted exact-SHA smoke, named Devpost page, and YouTube metadata while redacting private values; the post-submit phase additionally binds authenticated field readback to the final name, track, description hash, repository, demo, and video |

Passing fixture, source-tree, CI, and hosted checks proves the bounded product path, reviewer access, and its deterministic invariants. The Story and World workbenches additionally prove completed Codex CLI generation under a requested-model flag. Neither proves the actual serving model, independent GPT-5.6 attribution, human creator prose acceptance, quest support, or user productivity. Devpost submission is verified separately through authenticated readback.
...
### Gate 3 — Codex generation and product surface

**Status: PARTIAL.** The world-first Book 19 product path, frozen Table surface, and supporting Story Workbench all pass their local product gates. A user-authorized Codex CLI World run requested `gpt-5.6-terra`, completed four accepted narration turns across both endings, and failed closed on two earlier wording defects without mutating state. The transport reports only the requested model; actual model identity and response provenance remain unknown, and automated harness acceptance is not creator prose approval. The historical approval-bound evidence path remains closed exactly as recorded: primary `codex_cli_process_failed`, `retry-1` terminally rejected an unrecognized event/item type, and no further attempt is authorized.
...
It is not a retry of either historical evidence authority and does not produce Responses API identity. Its narrow claim is a completed ChatGPT-authenticated Codex CLI story run requesting `gpt-5.6-terra`. The browser's local World lane uses the same requested-model transport behind its loopback-only gate.
...
The primary sequence above is terminal and must not be rerun. The separately bound recovery sequence below was also consumed and is retained only as a reproducibility reference; do not execute it again:

```bash
npm run evidence:codex-cli:retry:review
npm run evidence:codex-cli:retry:approve -- --authority-sha <sha>
npm run evidence:codex-cli:retry:preflight
npm run evidence:codex-cli:retry:capture
```

`retry-1` was the only CLI retry. It used a distinct human-supplied authority SHA and could not accept the primary approval. Its terminal failure consumed that authority; no `retry-2` is registered.

The creator inspects the ignored packet before supplying its combined authority SHA. The adapter requests `gpt-5.6-sol` through the ChatGPT-authenticated Codex CLI, uses an empty read-only ephemeral workspace, accepts no tool events, and writes a terminal receipt after dispatch so the approval cannot be reused. This proves only the CLI transport when a real capture exists; it does not provide Responses API model or response identity.

**Go:** one sanitized real GPT-5.6 trace from a clearly named transport is verified; fixture/live and Responses/CLI identities cannot cross; the facilitator completes both steps without editing JSON; browser smoke and full current-tree recertification pass.
**No-Go:** label fixture output as live, relabel CLI evidence as a Responses response, reuse an approval after a terminal dispatch, expose a paid unauthenticated route, or show participant/character identity as if it were actual remote collaboration.

### Gate 4 — Evidence and release

**Status: PASS PER RELEASE RECORD.** The portable-world release runs evidence regeneration and verification, lint, typecheck, unit and integration tests, privacy scanning, production build, trace-privacy inspection, and production browser checks. The public video, private `/feedback` field, and authenticated Devpost confirmation are complete. Each new commit must still be recertified through GitHub CI, deployment, hosted exact-SHA smoke, and the ignored release record. Independently attributable serving-model evidence and human creator prose acceptance remain unclaimed rather than silently inferred.
...
**Go:** source-rights check, privacy/path/secret scan, claim parity, public video, `/feedback` ID, and final Devpost confirmation pass; the final release commit again passes fresh-clone CI and hosted exact-SHA smoke, then `submission:check` and `submission:check:post` both return PASS.
...
- world-first Book 19 loop with C intervention, offstage NPC response, The Loom, World Aftermath, World Codex, NPC cards, two earned endings, and Fork Compare from a shared checkpoint
...
2. run the preregistered same-model `default_instruction_control` versus `profiled` AB/BA probe with objective checks and a condition-masked creator rubric; make no cross-model superiority claim
...
The later Story and World Workbench runs are distinct user-authorized product-generation lanes, not repairs or relabelings of those receipts. Their automated evidence supports scene continuity, transport completion, and fail-closed state authority; serving-model identity and human creator writing verdict remain explicitly open.
