...
2. **Public repository and hosted fixture** — recommend a public GitHub repository after the README exists and the ignored release record matches the final clean HEAD, then a provider deployment whose `/api/health` commit matches public HEAD. Approval phrase: `공개 GitHub와 배포 승인`.
