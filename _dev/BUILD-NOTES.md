...
- Focused CLI verification passed at 7 files / 54 tests. After truth-surface synchronization, the full source-tree gate passed: evidence 7/7, ESLint, non-incremental TypeScript, 60 Vitest files / 429 tests, privacy scan over 237 public candidates, production build, and 10 clean Next trace manifests. A later exact-SHA deployment rehearsal exposed one stale smoke predicate: `/api/health` truthfully returned `liveEvidenceReadinessRecorded: false` for tracked `not_executed` readiness, while smoke incorrectly required `true`. Smoke now requires a present boolean signal, accepts honest `false`, and leaves completed live-proof enforcement to `submission:check`. That same rehearsal exposed local Playwright using `next dev` after the production build and changing tracked `next-env.d.ts`; release scripts now force `next start` and recheck clean exact-SHA source after browser exit. Per-commit exact-SHA authority remains externalized to the ignored release record.
