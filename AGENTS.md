...
- Core slice: facilitator-collected participant intents + creator-owned style profile → character-scoped retrieval → GPT-5.6 structured draft → hard validation → creator decision → canon overlay and deterministic state transition → canon/knowledge graph view → frozen replay.
...
7. The MVP includes a deterministic graph view derived from typed JSON, local facilitator-collected multi-intent composition, and a bounded two-step simulation. Do not claim a graph database, remote multi-user room, or long-running autonomous simulation.
...
Core fixture vertical slice and post-core truth audit verified. Participant/style contracts, deterministic retrieval and validators, world-aware creator gate, before/after graph descriptors, two-step simulation, replay engine, gated GPT-5.6 adapter, Table UI, sanitized fixture evidence, preregistered style-ablation protocol, production build, browser smoke, and fresh-copy rehearsal exist. A real GPT-5.6 integration trace, four-call style capture and creator ratings, final README, public remote/deployment, video, `/feedback` field, and Devpost submission remain separate release gates. Never promote fixture or protocol evidence into a live-model result.
